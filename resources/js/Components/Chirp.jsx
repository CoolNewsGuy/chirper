...
function DeleteChirpButton({ chirp }) {
  const form = useForm();
  /**
   *
   * @param {React.FormEvent<HTMLFormElement>} e
  ...
  function onSubmit(e) {
    ...
    form.delete(route("chirps.destroy", chirp), {
      onSuccess: () => alert("Deleted successfully"),
    });
  }

  return (
    <form onSubmit={onSubmit} action="chirps.destroy">
      <DangerButton className="absolute right-0 top-1/2 -translate-y-1/2">
        Delete
      </DangerButton>
    </form>
  );
